...
with open(args.output_csv, 'w', newline='') as file:
    ...
    #new code start
    # Use OpenSSL 3 library names for perf CSV columns
    field = ["kem", "sig", "hs", "hs1s", "cost_hs_server_ms", "cost_hs_client_ms", "server_libcrypto.so.3", "server_kernel.kallsyms", "server_libssl.so.3", "server_libc-2.31.so", "server_ixgbe", "server_python3.9", "client_libcrypto.so.3", "client_kernel.kallsyms", "client_libssl.so.3", "client_libc-2.31.so", "client_ixgbe", "client_python3.9"]
    #new code end
    ...
    for element in data:
        ...
        #new code start
        # Read server libcrypto.so.3 cost from perf data
        field.append(element["data_server"].get("libcrypto.so.3", (0,))[0])
        #new code end
        ...
        #new code start
        # Read server libssl.so.3 cost from perf data
        field.append(element["data_server"].get("libssl.so.3", (0,))[0])
        #new code end
        ...
        #new code start
        # Read client libcrypto.so.3 cost from perf data
        field.append(element["data_client"].get("libcrypto.so.3", (0,))[0])
        #new code end
        ...
        #new code start
        # Read client libssl.so.3 cost from perf data
        field.append(element["data_client"].get("libssl.so.3", (0,))[0])
        #new code end
